...
logger = logging.getLogger("engine.agentic")
...
async def _cove_verify(
    ...
    prompt = (
        ...
        "Ответ ассистента:\n{response}\n\n"
        "ПРАВИЛО ПРОВЕРКИ:\n"
        "1. ОТКЛОНИ ответ ТОЛЬКО если В контексте НЕТ ни одной статьи, которая хотя бы косвенно относится к вопросу.\n"
        "2. Если статья найдена но не даёт точного ответа — отвечай частично, цитируй статью и указывай что конкретное зависит от обстоятельств.\n"
        "3. НИКОГДА не возвращай «Информация не найдена» если в контексте есть хотя бы одна статья.\n"
        "4. ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ: Любой ответ (даже частичный) ДОЛЖЕН содержать ссылку на номер статьи и название кодекса/закона (например: (Статья X Кодекса РК)). Без этого ответ недопустим.\n\n"
        "Оцени ответ ассистента. Если он нарушает правила (например, говорит что информации нет, хотя статьи есть, ИЛИ не содержит ссылки на статью), напиши НОВЫЙ правильный ответ с соблюдением всех правил.\n"
...
async def invoke_agentic_qa(
    ...
    # 7. Deterministic citation guarantee (Rule 2) — runs LAST so a CoVe-rewritten answer
    # cannot bypass it. ensure_answer_citations is idempotent: if the answer already cites a
    # grounded article it is returned unchanged; otherwise the applicable article is appended.
    from engine.utils.ensure_citations import ensure_answer_citations

    result = ensure_answer_citations(result, source_documents, question=query)

...
from engine.retrieval.core import MinimalLegalRetriever
from engine.utils.circuit_breaker import CircuitBreaker, CircuitBreakerProxy
from engine.lifecycle_hooks import network_sensor
...
def _make_embeddings() -> PrefixedEmbeddings:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "запустите: python -m engine.scripts.download_models. "
            "Кэш: %s" % cache_folder
        ) from exc
...
def _load_bm25_chunks() -> list[Document] | None:
    ...
    try:
        if config.DOCUMENTS_DIR.exists():
            from engine.processing import prepare_data as _pd

...
TEST_QUERIES_PATH = Path(
    os.environ.get("LEGAL_RAG_TEST_QUERIES_PATH", "test_queries.json")
)
...
def _load_eval_questions() -> List[Dict[str, Any]]:
    if TEST_QUERIES_PATH.exists():
            ...
            normalized.append(
                {
                    ...
                    "relevant_articles": [
                        _normalize_article(x)
                        for x in relevant_articles
                        if _normalize_article(x)
                    ],
        ...
        if normalized:
            ...
            return normalized
        print(
            f"{TEST_QUERIES_PATH} найден, но не содержит валидных вопросов. Используется fallback-список."
        )
    else:
        print(
            f"{TEST_QUERIES_PATH} не найден. Используется встроенный список вопросов."
        )
...
def _compute_retrieval_metrics(
    ...
    rel_set = {
        _normalize_article(x) for x in relevant_articles if _normalize_article(x)
    }
    ranked = [
        _normalize_article(x) for x in retrieved_articles if _normalize_article(x)
    ]
...
def run_benchmark(
    ...
    *,
    ...
    try:
        try:
            import engine.retrieval.rag_chain as rag_chain_module
            from engine.retrieval.rag_chain import (
                get_llm,
                get_retriever,
                invoke_qa,
            )
    ...
    for i, item in enumerate(questions, 1):
        ...
        retrieved_articles = [
            _doc_article(d) for d in retrieved_docs if _doc_article(d)
        ]
        ...
        if use_judge:
            ...
            judge_context = "\n\n".join(
                d.page_content for d in (sources or retrieved_docs)[:10]
            )
            groundedness = evaluate_groundedness(
                answer=answer, context=judge_context, llm=llm
            )
    ...
    with open(json_path, "w", encoding="utf-8") as f:
        summary = {
            ...
            "avg_latency_generation_sec": _avg(
                [r.get("latency_generation_sec") for r in results]
            ),
            "avg_latency_judge_sec": _avg(
                [r.get("latency_judge_sec") for r in results]
            ),
            "avg_latency_total_sec": _avg(
                [r.get("latency_total_sec") for r in results]
            ),
        }
        json.dump(
            {

...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Offline BM25 benchmark against gold citations from XLSX."
    )
    parser.add_argument(
        "--xlsx",
    ...
    parser.add_argument(
        "--output",
        default="benchmark_results/gold_citations_local_bm25_benchmark.json",
...
def _normalize_article(value: str) -> str:
    ...
    if not text:
        return ""
    match = re.search(r"(?:ст\.?|статья|бап|article)\s*(\d{1,4}(?:-\d+)?)", text)
    if match:
...
def main() -> None:
    ...
    chunks = prepare_data.chunks
    retriever = BM25Retriever.from_documents(chunks, k=args.top_k)

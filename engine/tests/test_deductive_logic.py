...
from engine.retrieval.sherlock_engine import SherlockEngine
...
async def test_sherlock_uses_canonical_retriever(monkeypatch):
    ...
    monkeypatch.setattr(
        "engine.retrieval.sherlock_engine.rag_chain.get_retriever_for_coverage",
...
async def test_sherlock_offloads_llm_calls(monkeypatch):
    ...
    monkeypatch.setattr(
        "engine.retrieval.sherlock_engine.asyncio.to_thread",
        fake_to_thread,
    )
...
def test_invoke_qa_offline_mode_uses_extractively_retrieved_docs(monkeypatch):
    ...
    retriever.invoke.return_value = docs
    monkeypatch.setattr(
        "engine.retrieval.rag_chain._build_offline_bm25_retriever",
        lambda top_k: retriever,
    )
    monkeypatch.setattr(
        "engine.retrieval.rag_chain.get_llm",

...
# Пути: engine/core/config.py -> parent.parent = engine
_THIS_DIR = Path(__file__).resolve().parent
AI_SERVICE_DIR = _THIS_DIR.parent
BASE_DIR = AI_SERVICE_DIR.parent  # repo root (LegalRAG)


# Load .env from engine first, then repo root (python-dotenv always available in this project)
def _load_dotenv():
    try:
...
try:
    ...
    if not _pk:
        sys.exit(
            ...
            f"  e.g. source venv/bin/activate  (from LegalRAG) or  .venv/bin/activate  (from engine)\n"
            f"Original: {e}\n"
        )
    env_settings = type(
        "Env",

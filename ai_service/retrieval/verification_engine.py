...
class VerificationEngine:
    ...
    def classify_code(self, query: str) -> Dict:
        ...
        try:
            ...
            if selected == "ГК" and ("штраф" in query_lower or "полиция" in query_lower or "тишин" in query_lower):
                logger.info("Re-verifying: Switched from ГК to КоАП based on keywords 'штраф/полиция/тишина'")
    ...
    async def targeted_fetch(self, code_abbr: str, query: str, keywords: List[str]) -> List[Document]:
    ...
    def article_match(self, query: str, docs: List[Document]) -> Tuple[bool, List[Document]]:
...
class DeductiveReasoningSkill:
    ...
    def detect_conflicts(self, query: str, docs: List[Document]) -> Dict:
        ...
        context = "\n\n".join([f"Источник {d.metadata.get('code_ru')}, ст.{d.metadata.get('article_number')}: {d.page_content[:300]}..." for d in docs])
        try:
            resp = self.llm.invoke(CONFLICT_DETECTOR_PROMPT.format(query=query, context=context))
            content = resp.content if hasattr(resp, 'content') else str(resp)
            start = content.find('{')
            end = content.rfind('}') + 1
            return json.loads(content[start:end])
    ...
    async def run_deductive_cycle(self, query: str) -> Dict:
        ...
        conflicts = self.detect_conflicts(query, final_docs) if final_docs else {"has_conflict": False}

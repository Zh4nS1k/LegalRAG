...
if [ -f "./venv/bin/black" ]; then
    ./venv/bin/black --exclude "(\.venv|venv|\.git)" ai_service/ scripts/ tests/
else
    # Fallback to global black if available
    black --exclude "(\.venv|venv|\.git)" ai_service/ scripts/ tests/ || echo "Black is not installed. Skipping Python formatting."
fi

echo "Running Flake8 (Python linting)..."
if [ -f "./venv/bin/flake8" ]; then
    ./venv/bin/flake8 --exclude=".venv,venv,.git" ai_service/ scripts/ tests/
else
    flake8 --exclude=".venv,venv,.git" ai_service/ scripts/ tests/ || echo "Flake8 is not installed. Skipping Python linting."
fi
